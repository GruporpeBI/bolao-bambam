...
export default function GameCard({
  ...
  return (
    <Card
      variant="dark"
      className={`p-5 ${isOpen ? "ring-2 ring-[#F7EDE0]/80 border-[#F7EDE0]/70 shadow-[0_0_0_1px_rgba(247,237,224,0.25)]" : ""}`}
    >
      <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
        <div className="flex items-center gap-2">
          <span className="text-[#F7EDE0]/50 text-xs uppercase tracking-wider">
      ...
      <div className="flex items-center justify-between gap-4 my-4">
        ...
        <div className="flex items-center justify-end gap-2 flex-1">
          <span className="text-[#F7EDE0] font-semibold text-lg text-right">{homeTeam}</span>
          {homeFlagUrl && (
        ...
        {hasResult ? (
          <div className="flex flex-col items-center shrink-0">
            <span className="text-[#F7EDE0] font-black text-3xl tabular-nums leading-none">
              {game.home_score} – {game.away_score}
            </span>
            {poss && (
              <span className="text-[#F7EDE0]/35 text-[10px] mt-0.5 font-normal">
                {poss.team} {poss.pct}% posse
              </span>
            )}
          </div>
        ) : (
          <span className="text-[#F7EDE0] font-black text-2xl shrink-0">×</span>
        )}
        ...
        <div className="flex items-center justify-start gap-2 flex-1">
          ...
          <span className="text-[#F7EDE0] font-semibold text-lg text-left">{awayTeam}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          {isPastDeadline ? (
            ...
            <>
              <span className="text-[#F7EDE0]/50 text-xs shrink-0">Fecha em:</span>
      ...
      {!isPredictionDay && !existingPrediction && !isPastDeadline && (
        <p className="text-xs text-[#F7EDE0]/40 mt-2">
      ...
      {needsTournament && (
        <p className="text-xs text-[#F7EDE0]/70 border border-[#F7EDE0]/20 rounded-sm px-3 py-2 mt-3">
          Preencha os <span className="font-bold text-[#F7EDE0]">Palpites do Torneio</span> acima antes de enviar palpites de jogo.
        </p>
      )}

      {/* Palpite do usuário quando jogo já tem resultado */}
      {hasResult && existingPrediction && (
        <div className="mt-3 border-t border-[#F7EDE0]/10 pt-3">
          <p className="text-[#F7EDE0]/40 text-xs uppercase tracking-wider mb-1.5">
            Seu palpite
          </p>
          <div className="flex items-center gap-2">
            <span className="text-[#F7EDE0]/70 font-semibold text-base tabular-nums">
