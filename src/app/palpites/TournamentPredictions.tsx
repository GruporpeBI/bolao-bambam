...
function ScoreInput({
  ...
  return (
    <input
      ...
      placeholder="0"
      maxLength={2}
      className="w-14 text-center bg-[#5A1220] border border-[#F7EDE0]/30 text-[#F7EDE0] rounded-sm py-2 text-lg font-bold outline-none focus:border-[#F7EDE0] disabled:opacity-40"
    />
  );
}
...
function TeamSelect({
  ...
  return (
    <select
      ...
      disabled={disabled}
      className="bg-[#5A1220] border border-[#F7EDE0]/30 text-[#F7EDE0] rounded-sm px-3 py-2 text-sm outline-none focus:border-[#F7EDE0] disabled:opacity-40 min-w-[140px]"
    >
...
function WinnerBadge({ team, isDraw }: { team: string; isDraw: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 mt-2">
      <div className={`h-px flex-1 ${isDraw ? "bg-yellow-500/40" : "bg-[#F7EDE0]/20"}`} />
      <span className={`text-xs font-bold px-3 py-1 rounded-sm ${isDraw ? "bg-yellow-500/20 text-yellow-400 border border-yellow-500/40" : "bg-[#7D1A2E] text-[#F7EDE0] border border-[#F7EDE0]/30"}`}>
...
export default function TournamentPredictions({ disabled, existing, tournamentDeadlinePassed }: TournamentPredictionsProps) {
  ...
  if (tournamentDeadlinePassed && !existing) {
    return (
      <div className="border border-[#F7EDE0]/10 rounded-sm px-5 py-4 text-[#F7EDE0]/40 text-sm">
  ...
  const selectClass = "bg-[#5A1220] border border-[#F7EDE0]/30 text-[#F7EDE0] rounded-sm px-3 py-2 text-sm outline-none focus:border-[#F7EDE0] disabled:opacity-40";
  ...
  if (existing && lock) {
    ...
    const TeamName = ({ name }: { name: string }) => (
      <span className="flex items-center gap-1.5">
        <Flag name={name} />
        <span className="text-[#F7EDE0] font-semibold text-sm">{name}</span>
      </span>
    );

    return (
      <Card variant="dark" className="p-5 flex flex-col gap-5">
        <p className="text-xs text-[#F7EDE0]/50 uppercase tracking-wider">Palpites do Torneio — Enviados</p>
        <div className="flex flex-col gap-4">
          {[
            ...
            <div key={sf.label} className="flex flex-col gap-1.5">
              <span className="text-[#F7EDE0]/40 text-xs uppercase tracking-wider">{sf.label}</span>
              <div className="flex flex-wrap items-center gap-2">
                <TeamName name={sf.a} />
                <span className="text-[#F7EDE0] font-black">{sf.sa} × {sf.sb}</span>
                <TeamName name={sf.b} />
                <span className="flex items-center gap-1 text-xs bg-[#7D1A2E] text-[#F7EDE0] px-2 py-0.5 rounded-sm">
                  → <Flag name={sf.winner} /><span>{sf.winner}</span>
                </span>
              </div>
            </div>
          ))}
          <div className="border-t border-[#F7EDE0]/10 pt-3">
            <span className="text-[#F7EDE0]/40 text-xs uppercase tracking-wider">Final</span>
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <TeamName name={existing.finalist1} />
              <span className="text-[#F7EDE0] font-black">{existing.final_score_a} × {existing.final_score_b}</span>
              <TeamName name={existing.finalist2} />
            </div>
            {existing.possession_pred_final != null && (
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[#F7EDE0]/40 text-xs">Posse da final:</span>
                <span className="text-[#F7EDE0]/70 text-xs font-bold">{existing.possession_pred_final}% — {existing.finalist1}</span>
              </div>
            )}
            <div className="flex items-center gap-2 mt-2">
              <span className="text-[#F7EDE0]/40 text-xs">Campeão:</span>
              <span className="flex items-center gap-1.5 text-[#F7EDE0] font-bold">
        ...
        <p className="text-xs text-[#F7EDE0]/30">Palpites do torneio enviados e bloqueados.</p>
      </Card>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Card variant="dark" className="p-5 flex flex-col gap-7">
        <div>
          <p className="text-xs text-[#F7EDE0]/50 uppercase tracking-wider mb-0.5">Palpites do Torneio</p>
          <p className="text-[#F7EDE0]/30 text-xs">Obrigatório — preencha antes de enviar palpites de jogo.</p>
        </div>

        {/* ── SEMIFINAL 1 ── */}
        <div className="flex flex-col gap-3">
          <span className="text-sm font-bold text-[#F7EDE0] uppercase tracking-wider">Semifinal 1</span>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-[#F7EDE0] font-black text-xl">×</span>
        ...
        {/* ── SEMIFINAL 2 ── */}
        <div className="flex flex-col gap-3">
          <span className="text-sm font-bold text-[#F7EDE0] uppercase tracking-wider">Semifinal 2</span>
          <div className="flex flex-wrap items-center gap-3">
        ...
        {/* ── FINAL ── */}
        {finalReady && (
          <div className="flex flex-col gap-3 border-t border-[#F7EDE0]/20 pt-5">
            <span className="text-sm font-bold text-[#F7EDE0] uppercase tracking-wider"><img src="/api/flag/wc-logo" alt="Copa" style={{width:20,height:20,objectFit:"cover",borderRadius:3,flexShrink:0,display:"inline-block",verticalAlign:"middle"}} /> Final</span>
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-[#F7EDE0] font-semibold text-sm min-w-[120px]">{finalist1}</span>
              <ScoreInput value={finalA} onChange={setFinalA} disabled={lock} />
              <span className="text-[#F7EDE0] font-black text-xl">×</span>
              <ScoreInput value={finalB} onChange={setFinalB} disabled={lock} />
              <span className="text-[#F7EDE0] font-semibold text-sm min-w-[120px]">{finalist2}</span>
            </div>
            ...
            <div className="flex flex-col gap-2 mt-1">
              <span className="text-xs font-semibold text-[#F7EDE0] uppercase tracking-wider">Posse de bola — Final (%)</span>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="50"
                  maxLength={3}
                  className="w-16 text-center bg-[#5A1220] border border-[#F7EDE0]/30 text-[#F7EDE0] rounded-sm py-2 text-base font-bold outline-none focus:border-[#F7EDE0] disabled:opacity-40"
                />
                <span className="text-[#F7EDE0]/40 text-sm">% (mín. 50%)</span>
              </div>
            </div>

            {champion && (
              <div className="flex items-center gap-3 mt-1 bg-[#F7EDE0]/10 border border-[#F7EDE0]/30 rounded-sm px-4 py-3">
                <span className="text-[#F7EDE0]/60 text-sm">Campeão:</span>
                <span className="text-[#F7EDE0] font-black text-lg"><img src="/api/flag/wc-logo" alt="Copa" style={{width:20,height:20,objectFit:"cover",borderRadius:3,flexShrink:0,display:"inline-block",verticalAlign:"middle"}} /> {champion}</span>
              </div>
            )}
          </div>
        )}
        ...
        {!finalReady && (
          <div className="flex items-center gap-3 border-t border-[#F7EDE0]/10 pt-4">
            <div className="h-px flex-1 bg-[#F7EDE0]/10" />
            <span className="text-[#F7EDE0]/20 text-xs">Final disponível após preencher as semifinais</span>
            <div className="h-px flex-1 bg-[#F7EDE0]/10" />
          </div>
        )}
